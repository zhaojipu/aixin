...
import com.zds.base.code.common.HybridBinarizer;
import com.zds.base.code.decoding.CaptureActivityHandler;
import com.zds.base.code.decoding.InactivityTimer;
import com.zds.base.code.decoding.RGBLuminanceSource;
import com.zds.base.code.qrcode.QRCodeReader;
...
//        }
    ...
    private void putResult(Result result){
        ...
        if (result != null&&result.getText()!=null) {
             String data=  AESCipher.decrypt(result.getText());
             ScanResultInfo scanResultInfo = FastJsonUtil.getObject(data,ScanResultInfo.class);
             if (scanResultInfo!=null){
//                 ToastUtil.toast("成功");
                 if (SelfAppContext.getInstance().getUserInfoClass()!=null){
                     startActivity(new Intent(CaptureActivity.this,SelfAppContext.getInstance().getUserInfoClass()).putExtra("username",scanResultInfo));
                 }
                 CaptureActivity.this.finish();
             }else {

                 ToastUtil.toast("扫描失败！,请扫描爱聊好友二维码");
                 CaptureActivity.this.finish();
