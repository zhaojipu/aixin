...
public class EaseChatExtendMenu extends GridView {
     ...
     * @param listener    on click event of item
     */
    public void registerMenuItemNumber(String name, int number, int itemId, EaseChatExtendMenuItemClickListener listener) {
        ChatMenuItemModel item = new ChatMenuItemModel();
        item.name = name;
        item.number = number;
        item.id = itemId;
    ...
    class ChatMenuItem extends LinearLayout {
        ...
        private void init(Context context, AttributeSet attrs) {
            ...
            textView_number = findViewById(R.id.tv_number);
            ll_image = findViewById(R.id.ll_img);
        }
        ...
        public void setTvImage(int number) {
            ...
            ll_image.setVisibility(VISIBLE);
