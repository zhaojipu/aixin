 ...
 * 更新日期: 2017/11/30
 */
public class ChatRedAddPacket extends EaseChatRow {
    private TextView tv_message;
    private LinearLayout ll_container;
    private ImageView img_red;

    public ChatRedAddPacket(Context context, EMMessage message, int position, BaseAdapter adapter) {
        super(context, message, position, adapter);
    }

    @Override
    protected void onInflateView() {
        if (message.getStringAttribute(Constant.MSGTYPE, "").equals(Constant.ADDUSER)) {
    ...
    protected void onSetUpView() {
        ...
        if (message.getChatType().equals(EMMessage.ChatType.ChatRoom)) {
            ...
            String messagest = name + "加入了房间";
            if (name.length() > 0) {
                final SpannableStringBuilder sp = new SpannableStringBuilder(messagest);
                sp.setSpan(new ForegroundColorSpan(0xffFA9E3B), 0, name.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); //字体颜色
                tv_message.setText(sp);
            } else {
                tv_message.setText(messagest);
            }
        }
    }
