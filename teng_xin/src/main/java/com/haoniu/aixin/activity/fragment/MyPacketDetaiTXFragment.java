...
public class MyPacketDetaiTXFragment extends EaseBaseFragment {
    ...
    private void getDate() {
        ...
        map.put("pageSize", 20);
        ApiClient.requestNetHandle(getContext(), AppConfig.tixianjil, "", map, new ResultListener() {
            @Override
            public void onSuccess(String json, String msg) {
                List<TiXianInfo> tiXianInfos = FastJsonUtil.getList(json, "list", TiXianInfo.class);
                list.clear();
                if (tiXianInfos != null && tiXianInfos.size() > 0) {
                    list.addAll(tiXianInfos);
                    if (tiXianInfos.size()<20){
